...
class _MyTemplatesState extends State<MyTemplates> {
  ...
  void fetchData() async {
    if (homeController.templateList.value == null) {
      await homeController.fetchTemplates();
    }
    homeController.templateSearchList.value = List.from(homeController.templateList.value ?? []);
  }

  void _filterTemplates() {
    final templates = homeController.templateList.value ?? [];
    if (tag.isEmpty) {
      homeController.templateSearchList.value = templates;
    } else {
      homeController.templateSearchList.value = templates
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => TemplateEditor(
          ...
          onSave: () async {
            await TemplateApi.createTemplate(newTemplate.toStructuredData());
            print(newTemplate.toStructuredData());
            await homeController.fetchTemplates();
          },
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Obx((){
          return Text(
              'My Templates(${homeController.templateSearchList.value?.length??'NA'})',
              style: TextStyle(
        ...
        actions: [
          Obx(() {
            return homeController.isTemplateLoading.value
                ? SizedBox(
                    width: 20,
                    height: 20,
                ...
                : IconButton(
                    onPressed: () => homeController.fetchTemplates(),
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 20),
            Obx(() {
              if (homeController.isTemplateLoading.value) {
                return Expanded(
                  child: Center(
                    child: CircularProgressIndicator(),
                  ),
              ...
              final templates = homeController.templateSearchList.value ?? [];
